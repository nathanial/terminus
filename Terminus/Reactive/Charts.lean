...
where
  renderVerticalBarChart (data : Array BarData) (config : BarChartConfig) : IO RNode := do
  ...
  renderHorizontalBarChart (data : Array BarData) (config : BarChartConfig) : IO RNode := do
    let maxVal := computeBarMax data config.maxValue
...
def dynBarChart' (data : Reactive.Dynamic Spider (Array BarData)) (config : BarChartConfig := {})
  ...
  emitDynamic do
    ...
    if arr.isEmpty then
      ...
      let maxVal := computeBarMax arr config.maxValue
      -- For simplicity, just render horizontal bars in dynamic mode
      let chartWidth := config.size
      let maxLabelLen := arr.foldl (fun acc d => Nat.max acc d.label.length) 0
      let labelWidth := Nat.min maxLabelLen 10
...
def dynLineChart' (series : Reactive.Dynamic Spider (Array DataSeries))
  ...
  emitDynamic do
    let arr ← series.sample
    if arr.isEmpty || arr.all (·.data.isEmpty) then
      pure RNode.empty
    else
      -- Similar logic to static version
      let allMin := arr.foldl (fun acc s => if s.min < acc then s.min else acc) 1.0e38
      let allMax := arr.foldl (fun acc s => if s.max > acc then s.max else acc) (-1.0e38)
      let yMin := config.yMin.getD allMin
      let yMax := config.yMax.getD allMax
      let yRange := if yMax - yMin <= 0 then 1.0 else yMax - yMin

      let chartWidth := config.width - (if config.showYAxis then config.yAxisWidth else 0)
      let chartHeight := config.height - (if config.showXAxis then 1 else 0)

      let maxLen := arr.foldl (fun acc s => Nat.max acc s.data.size) 0
...
def dynPieChart' (data : Reactive.Dynamic Spider (Array PieSlice))
  ...
  emitDynamic do
    let arr ← data.sample
    let validData := arr.filter (·.value > 0)
    if validData.isEmpty then
      pure RNode.empty
    else
      let total := validData.foldl (fun acc s => acc + s.value) 0.0
